...
class CreateStaffPromotionTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('staff_promotion', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('company_company_id')->nullable()->index('FKF13AA2041F6798AB');

            $table->foreign(['agent_company_id'], 'FKF13AA204C6C7AD23')->references(['company_id'])->on('company')->onDelete('cascade');
            $table->foreign(['company_company_id'], 'FKF13AA2041F6798AB')->references(['company_id'])->on('company')->onDelete('cascade');
        });
    }
