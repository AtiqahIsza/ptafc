...
class CreateCardPointTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('card_point', function (Blueprint $table) {
            $table->unsignedBigInteger('point_id')->primary();
            $table->decimal('credit_amount')->nullable();
            $table->decimal('debit_amount')->nullable();
            $table->dateTime('point_date');
            $table->unsignedTinyInteger('type')->nullable();
            $table->unsignedBigInteger('card_card_id')->nullable()->index('FK3B04A7A11483D948');

            $table->foreign(['card_card_id'], 'FK3B04A7A11483D948')->references(['card_id'])->on('ticket_card')->onDelete('cascade');
        });
    }
