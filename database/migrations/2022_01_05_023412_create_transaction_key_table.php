...
class CreateTransactionKeyTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('transaction_key', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('agent_agent_id')->nullable()->index('FK4B4D017E1EDF5D0A');

            $table->foreign(['agent_agent_id'], 'FK4B4D017E1EDF5D0A')->references(['agent_id'])->on('reload_agent')->onDelete('cascade');
        });
    }
