...
class CreateTicketPromotionTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('ticket_promotion', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('region_id')->nullable()->index('FK12F50330F26F9C4D');
            ...
            $table->foreign(['region_id'], 'FK12F50330F26F9C4D')->references(['id'])->on('region_code')->onDelete('cascade');
            $table->unique(['region_id', 'start_date', 'end_date', 'from_amount', 'to_amount'], 'ticket_promo_idx');
        });
    }
