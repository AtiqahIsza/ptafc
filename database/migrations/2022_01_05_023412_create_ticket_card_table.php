...
class CreateTicketCardTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('ticket_card', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('region_region_id')->nullable()->index('FK1585D063F26F9C4D');

            $table->foreign(['region_region_id'], 'FK1585D063F26F9C4D')->references(['region_id'])->on('region_code')->onDelete('cascade');
            $table->foreign(['createdBy_agent_id'], 'FK1585D063C3139010')->references(['agent_id'])->on('reload_agent')->onDelete('cascade');
        });
    }
