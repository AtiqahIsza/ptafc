...
class CreatePdaTrackingHistoryTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('pda_tracking_history', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('bus_bus_id')->nullable()->index('FKCF230C1E7E2B6AE8');
            $table->unsignedBigInteger('pda_pda_id')->nullable()->index('FKCF230C1E53504BCC');
            $table->unsignedBigInteger('route_route_id')->nullable()->index('FKCF230C1E87F2551F');
            $table->string('speed', 16)->nullable();
            $table->integer('bus_scheduler_id')->nullable();
            $table->string('trip_generated_id', 16)->nullable();

            $table->foreign(['route_route_id'], 'FKCF230C1E87F2551F')->references(['route_id'])->on('route')->onDelete('cascade');
            $table->foreign(['bus_bus_id'], 'FKCF230C1E7E2B6AE8')->references(['bus_id'])->on('bus')->onDelete('cascade');
            $table->foreign(['pda_pda_id'], 'FKCF230C1E53504BCC')->references(['pda_id'])->on('pda_profile')->onDelete('cascade');
        });
    }
